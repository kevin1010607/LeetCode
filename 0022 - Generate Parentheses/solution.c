 ...
 */
void generate(char *buf, int idx, int left, int right, Vector *v){
    if(left){
        buf[idx] = '(';
        generate(buf, idx+1, left-1, right, v);
        buf[idx] = '\0';
    }
    if(right > left){
        buf[idx] = ')';
        generate(buf, idx+1, left, right-1, v);
        buf[idx] = '\0';
    }
    if(!left && !right) pushBack(v, buf);
}
char ** generateParenthesis(int n, int* returnSize){
    Vector *v = newVector(0, 105, 20, "");
    char buf[20] = {};
    generate(buf, 0, n, n, v);
    char **res = getArray(v);
