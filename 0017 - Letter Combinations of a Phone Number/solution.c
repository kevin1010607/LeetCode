 ...
 */
void solve(char *digits, int id, Vector *v, char m[][5], char *now){
    if(!digits[id]) {pushBack(v, now); return;}
    int idx = digits[id]-'0';
    for(int i = 0; m[idx][i]; i++){
        now[id] = m[idx][i];
        solve(digits, id+1, v, m, now);
    }
}
char ** letterCombinations(char * digits, int* returnSize){
    if(!*digits) {*returnSize = 0; return NULL;}
    const char m[10][5] = {"","","abc","def","ghi","jkl","mno","pqrs","tuv","wxyz"};
    Vector *v = newVector(0, 105, 5, "");
    solve(digits, 0, v, m, m[0]);
    char **res = getArray(v);
